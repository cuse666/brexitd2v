...
*/
  ...
  d3.csv("public/data/CovidSouthEast.csv").then(function (data) {
    data.forEach(d => {
      let tmp = {};
    ...
    */
    ...
    let showupText = svgChart
      ...
      .attr("data-label", d => d.label.slice(0))
      .text(d => twitterText[d.label.slice(0)])
    ...
    let textDateLabel = svgChart
      ...
      .attr("id", d => `textDateLabel-${d.label.slice(0)}`)
      .style("display", "none");
    ...
    let option_pauseSetting_msg = option_pauseSetting.append("p")
      ...
      .text("(Pause function requires at least TWO countries to be selected.)")
    ...
    function position(dot) {
      ...
      dot
        ...
        .style("display", function (d) {
          if (
            !isVisible(d) &&
            selectedLabel.findIndex(label => label === d.label.slice(0)) === -1
          ) {
            return "none";
          }
        });
    ...
    */
    ...
    //     // .filter(d => trueText.findIndex(text => text === d.label.slice(0)) >= 0 || (d.forward + 1 >= 800 || d.freq + 1 >= 50000))
    //     .style('opacity',function(d){
    //       if(d.forward>50&&d.freq>1400){
    //         return 1;
    //       }else{
    ...
    function textPosition(textData) {
      ...
      if (highlightColoredBubble) {
        text.style('opacity', function (d) {
          if (selectedLabel.length) {
            if (selectedLabel.includes(d.label.slice(0))) {
              return 1;
            } else {
              return 0;
            }
    ...
    function createDownsidePanel(labelSet) {
      ...
      downsideBlock.append("p")
        ...
        .html("Epidemic Timeline is the coloured dash bars, show the occurrence of the chosen country bubbles on the bubble chart. You can try to select from the country <b>CHECKBOX</b> on the left-hand side and pressing the <b>PLAY</b> button to see the movement of the bubbles.")
    ...
    function updateMask(selectedLabel) {
      ...
      if (selectedLabel.length === 0) {
        dots
          .filter(function (d, i) {
            return mouseoverDot === null || d.label.slice(0) === mouseoverDot;
          })
          .transition()
        ...
        if (highlightColoredBubble) {
          if (mouseoverDot !== null) {
            labels.filter((d) => {
              return d.label.slice(0) === mouseoverDot;
            })
              .transition()
              .duration(durationTime)
              .style("opacity", 1);
      ...
      dots
        .filter(function (d, i) {//未被选则的，鼠标没有悬停的所有话题
          return (
            ...
            (mouseoverDot === null || d.label.slice(0) !== mouseoverDot)
          );
        })
        .transition()
        .style("opacity", 0.1)
    ...
    document.getElementById("dataDownload").href = "public/data/CovidSouthEast.csv"
